		...
		case 2://group
			{
				...
				{
					CString s;
					s.Format("√руппа '%s' уже существует", pTVDispInfo->item.pszText);
		...
		case 0://effect
			{
				...
				{
					CString s;
					s.Format("Ёффект '%s' уже существует", pTVDispInfo->item.pszText);
					AfxMessageBox(s, MB_OK|MB_ICONWARNING);
		...
		case 1://emitter
			{
				...
				if (CString(pTVDispInfo->item.pszText)=="" || CString(pTVDispInfo->item.pszText)=="не св€зан")
				{
					AfxMessageBox("Ќе допустимое им€ эмиттера", MB_OK|MB_ICONWARNING);
					*pResult = 0;
				}
				else if(!pEffect->CheckName(pTVDispInfo->item.pszText))
				{
					CString s;
					s.Format("Ёмиттер '%s' уже существует", pTVDispInfo->item.pszText);
					AfxMessageBox(s, MB_OK|MB_ICONWARNING);
					*pResult = 0;
				}
				else
