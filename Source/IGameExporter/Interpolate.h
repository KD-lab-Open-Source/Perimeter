...
public:
	...
	{
		ITPL itpl;
		int interval_begin;//потом во float будет переводитьс€.
		int interval_size;
	...
	for(int iout=0;iout<out_data.size();iout++)
	{
		...
		for(int ibegin=one.interval_begin;ibegin<iend;)
		{
			...
			if(interval_size>=min_interval)
			{
				...
				one.a0=begin;//ѕотом на среднее по интервалу заменить
				Add(one);
...
void InterpolatePosition<T>::AddCubic(float delta)
{
	...
	for(int iout=0;iout<out_data.size();iout++)
	{
		...
		{//linear
/*		”же не нужно, так как в сплайнах на 1 точку больше стало.
