...
void terUnitSquad::repositionFormation(bool forceReposition)
{
	...
	if(!forceReposition && Units.size() == position_generator.counter()){ 
		// ѕросто пересчитать без паковки позиций
		SquadUnitList::iterator ui;
...
void terUnitSquad::repositionToAttack(AttackPoint& attackPoint, bool repeated)
{
	...
	if(Units.front()->attr().dynamicAttack){
		...
		xassert_s(min_radius < max_radius, (string("Ќедостаточный радиус атаки у ") + currentAttribute()->internalName()).c_str());
		...
		if(dist < min_radius)
			...
			return; // ÷ель в допустимой области - перестраиватьс€ не надо
...
public:
	...
	void operator()(terUnitBase* unit2)
	{
		...
		if(!ignoreField_ && including_cluster != unit2->includingCluster()) // закрыт куполом
			return;
		if(!(unit2->unitClass() & AttackClass)) // нельз€ стрел€ть
			return;
		if(unit2->isUnseen())
			return;
...
private:
	...
	/// режим сканировани€ дл€ базового сквада 
	/** 
		targets_[0] - цели дл€ солдат
		targets_[1] - цели дл€ офицеров
	*/
...
{
	if(!attack_points.empty()){ // ≈сть указанна€ цель.
