...
void terInterpolationReal::SetModel(const char* name,float scale)
{
	...
	//јтомарное изменение ObjectPoint
	cObjectNodeRoot* OldObjectPoint=ObjectPoint;
	ObjectPoint=NewObjectPoint;
	if(OldObjectPoint){
		if(gameShell->missionEditor())
		{
			// ривой фикс дл€ редактора миссий
			//логические объекты должны удал€тьс€ в логическом кванте!!!
...
{
	// Balmer temp пока пускай так, а потом должна быть интерпол€ци€
	if(volume_.fresh())
