...
struct EnvironmentalDamage
{
	...
	const DamageData damage(float damage_ratio) const 
	{
		...
		if(data.width != -1)
			data.width += xm::round(float(damageData[1].width - damageData[0].width) * damage_ratio);

		data.power += xm::round(float(damageData[1].power - damageData[0].power) * damage_ratio);
...
struct terDebrisData
{
	...
	int count;
	// если равно -1, то осколков будет count + xm::round(pow(radius / 4,1.5f));
	int countRnd;
