...
{
	MTAuto mtenter(&lock);
	if(TextureName==0||TextureName[0]==0) return 0; // им€ текстуры пустое
...
bool cTexLibrary::ReLoadTexture(cTexture* Texture,Vect2f kscale)
{
	...
	if(bump && !pos_bump)
	{
		//Ёта текстура никогда не должна использоватьс€, make small texture.
...
/*
	if(Option_FavoriteLoadDDS && !bump && !Texture->IsScaleTexture())
	{
		...
		int ret=ResourceFileRead(path_buffer,buf,size);///”течка пам€ти
		if(!ret)
		{
			...
			case 2:
				...
				break;
			default:
				//Ќеправильный dds файл. ¬ конце должен быть байт с alpha.
				Error(Texture);
				Texture->Release();
				return false;
			}
