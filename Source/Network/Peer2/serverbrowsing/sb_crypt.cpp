...
unsigned char GOAEncryptByte(GOACryptState *state, unsigned char b)
{
    ...
    state->ratchet += state->cards[state->rotor++];
    ...
    state->avalanche += state->cards[swaptemp];
	...
	*/
    state->last_cipher = b^state->cards[(state->cards[state->avalanche] + state->cards[state->rotor]) & 0xFF] ^
		state->cards[state->cards[(state->cards[state->last_plain] +
		state->cards[state->last_cipher] +
...
unsigned char GOADecryptByte(GOACryptState *state, unsigned char b)
{
	...
	*/
	...
	state->last_plain = b^state->cards[(state->cards[state->avalanche] + state->cards[state->rotor]) & 0xFF] ^
		state->cards[state->cards[(state->cards[state->last_plain] +
		state->cards[state->last_cipher] +
		state->cards[state->ratchet])&0xFF]];
    state->last_cipher = b;
