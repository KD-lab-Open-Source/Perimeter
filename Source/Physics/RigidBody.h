...
public:
	...
	void initPose(const Se3f& pose); // ¬ызывать при первой установке
	void initPose(const Se3f& pose, bool modify_z);

	void setPosition(const Vect3f& position) { pose_.trans() = position; matrix_.trans() = position; } // ƒл€ перемещени€
	...
	// States
	bool groundColliding() const { return ground_colliding; } // сталкиваетс€, стоит на земле
	bool chaosColliding() const { return chaos_colliding; } // сталкиваетс€ с хаосом
	float chaosCollidingFactor() const { return !flyingMode() ? chaosCollidingFactor_ : 0; } // сталкиваетс€ с хаосом 0..1
	int clusterColliding() const { return cluster_colliding; } // сталкиваетс€ с силовым полем кластера (ID)
	bool gravicapMode() const { return gravicap_mode; } // парит на воздушной подушке

	float velocityFactor() const { return velocity_factor; } // модул€ци€ скорости
...
private:
	...
	// States
	bool ground_colliding; // сталкиваетс€, стоит на земле
	bool chaos_colliding; // сталкиваетс€ с хаосом
	float chaosCollidingFactor_; // сталкиваетс€ с хаосом 0..1
	int cluster_colliding; // сталкиваетс€ с силовым полем кластера (ID)
	int including_cluster; // находитс€ внутри кластера (ID)
	bool gravicap_mode; // парит на воздушной подушке

	float velocity_factor; // модул€ци€ скорости
