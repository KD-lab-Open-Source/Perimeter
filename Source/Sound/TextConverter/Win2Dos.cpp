#include "StdAfx.h"
#include "Win2Dos.h"

static unsigned char win[256]=
{
	
};

void dos2win(LPSTR buffer)
{
	for(unsigned char* p=(unsigned char*)buffer;*p;p++)
		*p=win[*p];
}

bool printf_window_output=false;

void dosprintf(char *format, ...)
{
	va_list args;
	char    buffer[512];

	va_start(args,format);

	vsprintf(buffer,format,args);

	if(!printf_window_output)dos2win(buffer);
	printf(buffer);
}

void message_printf(char *format, ...)
{
	va_list args;
	char    buffer[512];

	va_start(args,format);

	vsprintf(buffer,format,args);

	if(!printf_window_output)
		MessageBox(NULL,buffer,"Text converter",MB_OK);
	else
		printf(buffer);
}
