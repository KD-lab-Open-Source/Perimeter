...
inline unsigned CRC(unsigned& crc, const char* str)
{
	do{
		unsigned x = *str;
		crc ^= x >> 3;
...
inline unsigned CRC(unsigned& crc, unsigned x)
{
	crc ^= x >> 3;
	crc ^= x << 28;
	crc ^= crc >> 3;
...
inline string variable_name(const char* name)
{
	string varName = name;
...
inline string template_name(const string& template_, const string& argument)
{
	if(argument[argument.size() - 1] != '>')
		return template_ + "<" + argument + ">";
	else
		return template_ + "<" + argument + " >";
}
