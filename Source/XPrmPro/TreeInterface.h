...
public:
	enum EditType {
		...
		HIDDEN,		// Игнорировать данный узел и его детей
	};

	typedef const TreeNode* (*TreeNodeFunc)(int typeIndex);
	typedef const char* (*CustomValueFunc)(HWND hWndParent, const TreeNode* treeNode);
	...
	explicit TreeNode(const char* name) 
		: name_(name), value_(""), type_(""), editType_(STATIC), comboList_(0), 
		defaultTreeNode_(0), defaultTreeNodeFunc_(0), customValueFunc_(0),
		parent_(0), expanded_(false), color_(0) {}
	...
	void setHidden() { editType_ = HIDDEN; }
	bool hidden() const { return editType() == HIDDEN; }
	...
	const char* customValue(HWND hWndParent) const { 
		return customValueFunc_ ? customValueFunc_(hWndParent, this) : 0; 
	}
	...
	//-----------------------------------------------------
	typedef std::vector<int> ColorList;

	void setColors(const ColorList& colors) {
		typedef std::vector<TreeNode*> TreeNodeList;
		TreeNodeList source, target;
		source.push_back(this);
		ColorList::const_iterator ci = colors.begin();
		while(!source.empty()){
			target.clear();
			TreeNodeList::iterator ni;
			FOR_EACH(source, ni){
				if(ci == colors.end())
					return;
				(*ni)->setColor(*ci);
	...
	void getColors(ColorList& colors) const 
	{
		colors.clear();
		std::back_insert_iterator<ColorList> inputColor = std::back_inserter(colors);
