...
void vrtMap::UndoDispatcher_PutPreChangedArea(int xL, int yT, int xR, int yB)
{
	...
	if(curPreCA!=preCAs.end()) preCAs.erase(curPreCA, preCAs.end()); //”даление не нужных элементов
	//ѕроверка на переполнение буфера и удаление 1-х элементов
	while((UNDO_REDO_BUFFER_SIZE+size) > MAX_SIZE_UNDO_REDO_BUFFER){
		if(preCAs.begin()==preCAs.end())ErrH.Abort("—писок пустой а UNDO_REDO_BUFFER_SIZE==", XERR_USER, UNDO_REDO_BUFFER_SIZE);
		preCAs.erase(preCAs.begin());
	}
	preCAs.push_back();
	curPreCA=preCAs.end();
	curPreCA--;//“еперь указывет на последний элемент
