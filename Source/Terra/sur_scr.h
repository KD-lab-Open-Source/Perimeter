...
struct s_row { //: s_el 
	vector<s_el> el;
	int type;
	int line_width;//Ўирина текстовой строки
	char name[40];
	int d_int; //default
	double d_double;
	char* d_char;
...
//¬ставка в конкретную позицию
	...
	void ins_el(int poz, char* v){ 
		el.insert(el.begin()+poz,1);
		char* k=new char[line_width+1]; //¬ыдел€ем пам€ть под строку 
		el[poz].v_char=k; //ѕомещаем указатель под выделенную пам€ть в массив. Ёти сложности необходимы т.к. ¬ массиве хран€тс€ указатели а не строки
		set_el(poz, v);
	};
...
struct s_sur_scr  { //: s_row
	...
	int beg_num_rows; //ƒл€ конвертации скрипта необходимо запомнить кол-во старых строк

	s_sur_scr() {
		numbers_rows=0;
		numbers_cell=0;
		current_cell=-1;//Ќачальное значение т.к. сразу пребавл€етс€ 1 дл€ вставки в заданную позицию
#ifdef _SURMAP2_
		draw_seq=0;
#endif
	};
	...
	void new_row() {};
	void new_col() {};
	void addIset_default(int x, int y){ //ƒобавление CELL и заполнение ее defaul-значени€ми
		for(int i=0; i<numbers_rows; i++) row[i].add_el_d();//row X и Y заполн€ютс€ передаваемыми X и Y
		row[0].set_el(numbers_cell, x);
		row[1].set_el(numbers_cell, y);
		current_cell=numbers_cell;
		numbers_cell++;
	};
	void addIset_default(int x, int y, int alpha){ //ƒобавление CELL и заполнение ее defaul-значени€ми
		for(int i=0; i<numbers_rows; i++) row[i].add_el_d();//row X и Y заполн€ютс€ передаваемыми X и Y
		row[0].set_el(numbers_cell, x);
		row[1].set_el(numbers_cell, y);
	...
	void insIset_default(int num_ins, int x, int y){ //ƒобавление CELL и заполнение ее defaul-значени€ми
		if(num_ins > numbers_cell) ErrH.Abort("Error in SurScr(insert element)");
		for(int i=0; i<numbers_rows; i++) row[i].ins_el_d(num_ins);//row X и Y заполн€ютс€ передаваемыми X и Y
		row[0].set_el(num_ins, x);
		row[1].set_el(num_ins, y);
		current_cell=num_ins;
		numbers_cell++;
	};
	void insIset_default(int num_ins, int x, int y, int alpha){ //ƒобавление CELL и заполнение ее defaul-значени€ми
		if(num_ins > numbers_cell) ErrH.Abort("Error in SurScr(insert element)");
		for(int i=0; i<numbers_rows; i++) row[i].ins_el_d(num_ins);//row X и Y заполн€ютс€ передаваемыми X и Y
		row[0].set_el(num_ins, x);
		row[1].set_el(num_ins, y);
...
#ifdef _SURMAP2_
	char draw_seq; //ѕоказ последовательности поинтов
	void drawCell(){
