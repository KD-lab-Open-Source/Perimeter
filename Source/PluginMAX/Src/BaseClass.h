...
public: 
	...
	inline void Detach(int number)
	{ // ищет указатель элемент Base в списке и удал€ет его из списка
		if(number>=length()) return;
...
//		memset(this,0,sizeof(cBaseDynArrayPointer)); 
	...
	{ // добавл€ет указатель в конец списка
	...
	{ // удал€ет из списка элемент с номером number, и удал€ет сам эелемент с номером
