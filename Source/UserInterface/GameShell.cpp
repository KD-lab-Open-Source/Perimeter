...
//			_shellIconManager.SetModalWnd(SQSH_MM_SPLASH1);
			_shellIconManager.AddDynamicHandler(showReels, CBCODE_QUANT); //ждать пока не слетитс€
		} else {
...
//			hide_debug_window();
	...
	case 'O' | KBD_CTRL: {
		...
		if(openFileDialog(saveName, missionEditor() ? MISSIONS_PATH : "Resource\\Saves", "spg", "Mission Name")){
			...
			if(pos != string::npos)
				saveName.erase(0, pos);
			//Ќесколько кривой участок кода, не будет работать с HT
			HTManager::instance()->GameClose();
...
void CShellLogicDispatcher::init()
{
	...
	//камера и сцена дл€ моделей в окошке
...
//	float _small_camera_rect_dy  = small_camera_rect_dy*float(terScreenSizeY)/2.f;
	...
	m_hCamera->SetFrustum(                          // устанавливаетс€ пирамида видимости
		...
		&sRectangle4f(-_small_camera_rect_dx, -_small_camera_rect_dy, 
						_small_camera_rect_dx, _small_camera_rect_dy),// видима€ область камеры
		&Vect2f(1.0f, 1.0f),                        // фокус камеры
		&Vect2f(30.0f, 10000.0f)                    // ближайший и дальний z-плоскости отсечени€
		);
...
void GameShell::editParameters()
{
	...
	const char* header = "«аголовок миссии";
	const char* mission = "ћисси€";
	const char* missionAll = "ћисси€ все данные";
	const char* debugPrm = "Debug.prm";
	const char* global = "√лобальные параметры";
	const char* attribute = "јтрибуты";
	const char* sounds = "«вуки";
	const char* interface_ = "»нтерфейс";
	const char* physics = "‘изические параметры";
